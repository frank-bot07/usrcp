...
| P2-1 em dashes | Stripped from all stream sources, README, package.json, the two usrcp-local lines I authored, and this tasks doc. **Scope**: only lines authored in this branch. Pre-existing em dashes in `packages/usrcp-local/src/server.ts` and `packages/usrcp-local/src/ledger/core.ts` are outside the diff and are intentionally left alone; a branch-wide sweep is a separate PR. (Codex round-2 P2.) |
...
Codex round 2 returned 1 P0 + 1 P1 + 1 P2. All addressed in commit following this section:

| Finding | Resolution |
|---|---|
| P0-1 stream_thread leaks thread-level metadata when all events are out-of-scope | Rewrote handler so scoped + zero-in-scope-events short-circuits to `status:"not_found"` BEFORE loading or decrypting the thread row. When some events are in-scope, surfaces/entity_refs/first_ts/last_ts are derived from the in-scope events themselves (not from the thread row). Each event now carries its own decrypted `entity_refs` through this path. Two regression tests in `scope-enforcement.test.ts`: (a) telegram-only thread queried from discord-scoped server -> not_found, no metadata, no SECRET_PROJECT_ID in JSON; (b) mixed-scope thread returns only in-scope event AND derives entity_refs from it (SECRETIVE_ENTITY visible only on the telegram event is NOT exposed). |
