/**
 * Reader pipeline: user @-mentions the bot → bot replies with context
 * drawn from the user's USRCP ledger (cross-channel + channel-local).
 *
 * buildSystemPrompt is exported separately from composeReply so tests
...
export function buildSystemPrompt(
  ...
  return [
    `You are an agent helping ${displayName} via Discord. Be concise.`,
    "",
    "User profile:",
    ...
    "",
    `Recent activity in THIS Discord channel (last ${CHANNEL_TIMELINE_LAST_N}):`,
    ...
    "(Discord messages, not essays). Do not invent facts not present in",
    "the activity log.",
  ].join("\n");
}
  ...
  ledger.appendEvent(
    {
      ...
      outcome: "success",
      detail: {
        responding_to_message_id: msg.id,
        raw_content: replyText,
      },
      tags: ["chat", "agent_reply", `channel:${msg.channel.name ?? msg.channel.id}`],
      channel_id: msg.channel.id,
      thread_id: msg.thread?.id,
      external_user_id: msg.author.id, // the user the bot is replying to
    },
    "discord",
    // Reply idempotency: tied to the triggering message ID so a gateway
    // replay of the same mention doesn't double-post the same reply.
