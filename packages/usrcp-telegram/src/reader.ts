/**
 * Reader pipeline: user @-mentions bot / DMs / replies-to-bot → bot replies
 * with context drawn from the user's USRCP ledger (cross-channel + chat-local).
 *
...
export function buildSystemPrompt(
  ...
  return [
    `You are an agent helping ${displayName} via Telegram. Be concise.`,
    "",
    "User profile:",
    ...
    "",
    `Recent activity in THIS Telegram chat (last ${CHANNEL_TIMELINE_LAST_N}):`,
    ...
    "(Telegram messages, not essays). Do not invent facts not present in",
    "the activity log.",
  ].join("\n");
}
...
export async function composeAndReply(
  ...
  ledger.appendEvent(
    ...
    "telegram",
    // Reply idempotency: tied to the triggering message's idempotency key so
    // a polling replay of the same update doesn't double-post the same reply.
    `telegram-reply:${msg.channel.id}:${msg.id}`,
    "telegram-bot"
  );
