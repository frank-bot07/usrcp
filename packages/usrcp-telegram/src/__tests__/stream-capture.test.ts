...
beforeEach(() => {
  ...
  tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), "usrcp-telegram-stream-"));
...
describe("Telegram stream-capture filtering", () => {
  ...
  it("captures outbound with side='outbound'", async () => {
    const r = await captureMessageToStream(
      streamClient,
      mkMsg({ content: "self", channel: { id: CHAT_A, name: "group" } }),
      config
    );
    expect(r.captured).toBe(true);
  ...
  it("skips bot messages", async () => {
    const r = await captureMessageToStream(
      streamClient,
      mkMsg({ author: { id: "bot", bot: true }, channel: { id: CHAT_A, name: "g" } }),
      config
    );
  ...
  it("skips non-allowlisted chats", async () => {
    const r = await captureMessageToStream(
      streamClient,
      mkMsg({ channel: { id: CHAT_UNLISTED, name: "off" } }),
      config
    );
    expect(r.captured).toBe(false);
  ...
  it("skips empty messages", async () => {
    const r = await captureMessageToStream(
      streamClient,
      mkMsg({ content: "  ", channel: { id: CHAT_A, name: "g" } }),
      config
    );
    expect(r.captured).toBe(false);
...
describe("resolveMode dispatch (Telegram)", () => {
