...
describe("Advanced Key Rotation", () => {
  ...
  // NOTE: the "resumes adapter-config rotation from a checkpoint on Ledger
  // open" test moved to usrcp-local (adapter-rotation-recovery.test.ts). It
  // exercises the integration of the core ledger's open-time recovery hook with
  // usrcp-local's adapter-rotation system, which only exists in usrcp-local.
});
